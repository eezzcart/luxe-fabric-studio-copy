        ...
        try {
          if (!cartId) {
            ...
            if (result.success) {
              ...
              set({
                items: current.map((i) => (i.variantId === item.variantId ? { ...i, quantity: newQty } : i)),
                isDrawerOpen: true,
                pulseVariantId: item.variantId,
              });
          ...
          setTimeout(() => set({ pulseVariantId: null }), 900);
        } finally {
          set({ isLoading: false });
        }
      },
